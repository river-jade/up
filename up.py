...
def convert_json_to_csv(start_date, csv_file):
    ...
    with open(csv_file, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['Time', 'BSB / Account Number', 'Account name', 'Transaction Type', 'Payee', 'Description', 'Category', 'Tags', 'Subtotal (AUD)', 'Currency', 'Subtotal (Transaction Currency)', 'Round Up (AUD)', 'Total (AUD)', 'Payment Method', 'Settled Date'])
        ...
        while True:
            ...
            for transaction in transactions:
                ...
                currency = maybe(transaction['attributes'])['foreignAmount']['currencyCode'].or_else(transaction['attributes']['amount']['currencyCode'])
                subtotal_transaction_currency = maybe(transaction['attributes'])['foreignAmount']['value'].or_else(subtotal_aud)
                ...
                settled_date = transaction['attributes']['settledAt']

                writer.writerow([transaction_time, '', '', '', payee, description, category, tags, subtotal_aud, currency, subtotal_transaction_currency, round_up_aud, total_aud, payment_method, settled_date])
...
if __name__ == "__main__":
    ...
    csv_file = 'up_transactions.csv'
    convert_json_to_csv(start_date, csv_file)
